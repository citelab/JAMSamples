...
function processArrayData(dataArray, stream){
    ...
    transfer(testData, myNetwork.toJSON()); //send to outflow
}

function transfer(testData, network){
    myFlow.push(network);   //send the network to the next application for prediction
}
