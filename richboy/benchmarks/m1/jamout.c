...
p = strtok(0, ",");
char *_class = p;
jamdata_log_to_server("global", "sensorData", jamdata_encode("ffsi", "sd_front", sd_front, "sd_left", sd_left, "_class", _class, "nodeID", nodeID), 1);
usleep(sendWait);
}
gettimeofday(&tv1, 0);
trackers[1].text = "";
trackers[1].tv1 = tv1;
jamdata_log_to_server("global", "sensorData", jamdata_encode("ffsi", "sd_front", "", "sd_left", "", "_class", "", "nodeID", nodeID), 1);
fclose(fp);
if(line) free(line);
}
