        ...
        if( strcmp(tempNodeID, nodeID) == 0 ){//strcmp(announcement.nodeID, nodeID) == 0
            ...
            if( status == 0 ){
                ...
                FILE *f = fopen(fileName, "a");
                 ...
                 char timepack[100];
                  snprintf(timepack,sizeof(timepack),"%lu,%lu,[Node: %s],end", tv1.tv_sec, tv1.tv_usec, nodeID);
                  timing = timepack;
            }
    ...
    do{
        ...
        if( endIndex != 0 ){//if a job was sent back
            ...
            while ((read = getline(&line, &len, fp)) != -1) {
                ...
                if( val == 0 )//if this will be completed
                    tracking++; //This will be how we know which node's processed data completed

                char timepack[100];
                snprintf(timepack,sizeof(timepack),"%lu,%lu,[Node: %s],[ID: %d],[Track: %d],start", tv1.tv_sec, tv1.tv_usec, nodeID, id, tracking);
