...
jnode.jcond.set('devonly', { source: 'eval(jcondContext("sys.type") == "device")', code: 0, cback: null, bcasts: [] });
...
function askforId() {
jnode.machAsyncExec("askforId", [  ], "jcond.get('devonly').source", 0, "jcond.get('devonly').bcasts", "jcond.get('devonly').cback");
}
function runAtDevice() {
jnode.machAsyncExec("runAtDevice", [  ], "jcond.get('devonly').source && jcond.get('matchid').source", 16, "jcond.get('devonly').bcasts", "jcond.get('devonly').cback");
}
